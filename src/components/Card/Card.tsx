...
const Card: React.FC<CardProps> = ({ nasaImage }) => {
  ...
  const cardhandler = React.useMemo(() => ({
    like: (): void => {
      if (!liked) {
  ...
  }), [dispatch, liked, nasaImage]);
  return (
    <CardStyles>
      <CardHeader />
      <CardImage cardImageSrc={nasaImage.url} cardImageTitle={nasaImage.title} liked={liked} />
      <CardButtonContainer>
        <CardButton type="button" aria-label="Like post" onClick={cardhandler.like}>
          <img src={liked ? RedHeart : Heart} alt="heart icon" />
        </CardButton>
        <CardButton aria-label="Share link" type="button" onClick={cardhandler.copyToClipboard}>
      ...
      />
      <Toast copied={copied} showToastHandler={setCopied} />
    </CardStyles>
  );
};
